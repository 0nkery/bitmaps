...
bitops_for_big!(7);
bitops_for_big!(8);
bitops_for_big!(50000);

pub trait InitDefault {
    fn init() -> Self;
}

impl InitDefault for bool {
    fn init() -> Self {
        false
    }
}

macro_rules! init_default_for {
    ($result:ty) => {
        impl InitDefault for $result {
            fn init() -> Self {
                0
            }
        }
...
init_default_for!(u8);
init_default_for!(u16);
init_default_for!(u32);
init_default_for!(u64);
init_default_for!(u128);

impl<const N: usize> InitDefault for [u128; N] {
    fn init() -> Self {
        [0; N]
    }
}
...
pub trait Bits {
    ...
    type Store: BitOps + InitDefault + Copy + PartialEq + Debug;
